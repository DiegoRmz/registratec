...
if(request.getParameter("action") != null){
    switch(request.getParameter("action")){
        ...
        case "eventpasado":
            String eventosssss =  profesorad.getEventosPasados();
            PrintWriter impresora1 = response.getWriter();
            response.setContentType("text/plain\n\n");
            impresora1.println(eventosssss);
            impresora1.close();
        
        case "todoeventos":
            String eventstr = profesorad.getEventoTodo();
            PrintWriter impresora2 = response.getWriter();
            response.setContentType("text/plain\n\n");
            impresora2.println(eventstr);
            impresora2.close();

        case "getsingleevent":
            String resultGetEvent = profesorad.consultaEventoPorId(Integer.parseInt(request.getParameter("id")));
            PrintWriter pwGetEvent = response.getWriter();
            response.setContentType("text/plain\n\n");
            pwGetEvent.println(resultGetEvent);
            pwGetEvent.close();

        case "deletevent":
            String resultDeleteEvent = profesorad.borrarEvento(Integer.parseInt(request.getParameter("id")));
            PrintWriter pwDeleteEvent = response.getWriter();
            response.setContentType("text/plain\n\n");
            pwDeleteEvent.println(resultDeleteEvent);
            pwDeleteEvent.close();

        case "editarevento":
            String resultEditEvent = profesorad.editEvento(eventodp);
            PrintWriter pwEditEvent = response.getWriter();
            response.setContentType("text/plain\n\n");
            pwEditEvent.println(resultEditEvent);
            pwEditEvent.close();

        case "deleteponente":
            String resultDeletePonente = profesorad.borrarPonente(Integer.parseInt(request.getParameter("id")));
            PrintWriter pwDeletePonente = response.getWriter();
            response.setContentType("text/plain\n\n");
            pwDeletePonente.println(resultDeletePonente);
            pwDeletePonente.close();
        
        case "getsinglespeaker":
            String resultGetPonent = profesorad.obtenerPonente(Integer.parseInt(request.getParameter("id")));
            PrintWriter pwGetPonent = response.getWriter();
            response.setContentType("text/plain\n\n");
            pwGetPonent.println(resultGetPonent);
            pwGetPonent.close();

        case "editspeaker":
            String resultEditSpeaker = profesorad.editPonente(ponentedp);
            PrintWriter pwEditPonent = response.getWriter();
            response.setContentType("text/plain\n\n");
            pwEditPonent.println(resultEditSpeaker);
            pwEditPonent.close();
